/*
Copyright Ignacio Castaño <castanyo@yahoo.es> 2006
Based on NVIDIA Mesh Mender library.
...
public:
	virtual bool CanSmooth(Triangle * t1, Triangle * t2, const float & minCreaseAngle)
	{
		...
		Vector3 tmp1 = normalize(t1->normal);
		Vector3 tmp2 = normalize(t2->normal);
...
public:
	virtual bool CanSmooth(Triangle * t1, Triangle * t2, const float & minCreaseAngle)
	{
		...
		//for checking the angle, we want these to be normalized,
		//they may not be for whatever reason
		Vector3 tmp1 = normalize(t1->tangent, 0.0f);
		Vector3 tmp2 = normalize(t2->tangent, 0.0f);

		if (dot(tmp1, tmp2) >= minCreaseAngle)
		{
			return true;
		}
		else if( ( tmp1 == Vector3(zero) ) && ( tmp2 == Vector3(zero) ) )
		{
			// check for them both being null, then they are 
			// welcome to smooth no matter what the minCreaseAngle is
			return true;
...
public:
	virtual bool CanSmooth(Triangle * t1, Triangle * t2, const float & minCreaseAngle)
	{
		...
		//for checking the angle, we want these to be normalized,
		//they may not be for whatever reason
		Vector3 tmp1 = normalize(t1->binormal, 0.0f);
		Vector3 tmp2 = normalize(t2->binormal, 0.0f);
								  ...
								  const NormalCalcOption computeNormals)
{
	...
	{ 
		...
		{
			Vector3 normalizedNorm = normalize(t.normal);
			Vector3 finalNorm = (normalizedNorm * (1.0f - WeightNormalsByArea)) + (t.normal * WeightNormalsByArea);
			t.normal = finalNorm;
		}
	}
						...
						std::vector< Vertex >&   theVerts )
{
	...
	for(size_t i = 0 ; i < len ; ++ i )
	{
		...
		Vector3 newT = tmpTan - (dot(tmpNorm, tmpTan) * tmpNorm );
		Vector3 newB = tmpBin - (dot(tmpNorm, tmpBin) * tmpNorm) - (dot(newT, tmpBin) * newT);

		theVerts[i].tangent = normalizeSafe(newT, Vector3(zero), 0.0f);
		theVerts[i].binormal = normalizeSafe(newB, Vector3(zero), 0.0f);
                               ...
                               Vector3& binormal) const
{
	...
	tangent.set(t2*P.x() - t1*Q.x(), t2*P.y() - t1*Q.y(), t2*P.z() - t1*Q.z());
	tangent = tmp * tangent;

	binormal.set(s1*Q.x() - s2*P.x(), s1*Q.y() - s2*P.y(), s1*Q.z() - s2*P.z());
