...
namespace
{
	...
	{
		return nv::min(nv::min(a, b), c);
	}
	
	static inline int max(int a, int b, int c)
	{
		return nv::max(nv::max(a, b), c);
	}
...
bool nv::Raster::drawTriangle(bool antialias, Vector2::Arg extents, const Vector2 v[3], SamplingCallback cb, void * param)
{
...
bool nv::Raster::drawQuad(bool antialias, Vector2::Arg extents, const Vector2 v[4], SamplingCallback cb, void * param)
{
	...
	if (sign0 == sign1) {
		...
		if (tri0.valid && tri1.valid) {
			if (antialias) {
				tri0.drawAA(extents, cb, param);
				tri1.drawAA(extents, cb, param);
			} else {
				tri0.draw(extents, cb, param);
				tri1.draw(extents, cb, param);
			}
		}
	}
	else
