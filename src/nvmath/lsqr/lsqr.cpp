...
#include "Pi/Core/Core.h"
#include "Pi/Core/Debug.h"
#include "lsqr.h"
...
lvec *alloc_lvec( int  lvec_size )
{
  ...
  lng_vec->elements = (int *) malloc( (lvec_size) * sizeof(int) );
...
lsqr_input *alloc_lsqr_in( int  max_num_rows,
			   int  max_num_cols  )
{
...
lsqr_output *alloc_lsqr_out( int  max_num_rows,
			     int  max_num_cols  )
{
  lsqr_output *out_struct;
...
lsqr_work *alloc_lsqr_wrk( int  max_num_rows,
		           int  max_num_cols )
{
  lsqr_work *wrk_struct;
...
/*          num_iter,*/
  ...
  if( input->lsqr_fp_out != NULL )
	...
	The matrix A has %7i rows and %7d columns\n\
	The damping parameter is\tDAMP = %10.2e\n\
	ATOL = %10.2e\t\tCONDLIM = %10.2e\n\
	BTOL = %10.2e\t\tITERLIM = %10d\n\n",
